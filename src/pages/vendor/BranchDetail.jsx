...
function useItemStocks(items, shopId, branchId){
    const [stockData, setStockData] = useState({})
    const [isLoadingStocks, setIsLoadingStocks] = useState(false)

    useEffect(() => {
        const getItemStocks = async () => {
            if (!items || items.length === 0 || !shopId || !branchId) return
...
function BranchDetail() {
    ...
    const { stockData, isLoadingStocks } = useItemStocks(items, shopId, branchId)
                ...
                <div>
                    {!loadingShops && !shopsError && shops.length > 0 && (
                        <button
                            ...
                            className="bg-primary hover:bg-primary-700  py-2 px-4 rounded-lg flex items-center shadow-md transition-all hover:shadow-lg"
                        >
                ...
                {!loadingShops && !shopsError && shops.length === 0 && (
                    <div className="bg-white rounded-lg shadow-md p-8 text-center">
                        ...
                        <Link to="/vendor/dashboard"
                            className="bg-primary hover:bg-primary-700  py-2 px-4 rounded-lg shadow-md hover:shadow-lg transition-all inline-block">
                            Create a Shop
                        </Link>
                    </div>
                )}
                                ...
                                <>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                                            ...
                                            <div className="bg-green-50 p-4 rounded-lg">
                                                <div className="flex items-center">
                                                    ...
                                                    <div className="ml-3">
                                                        <p className="text-sm font-medium text-green-600">Local In Stock</p>
                                                        <p className="text-2xl font-bold text-green-900">
                                                            {items.filter(item => item.stock_qty > 0).length}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="bg-yellow-50 p-4 rounded-lg">
                                                <div className="flex items-center">
                                                    ...
                                                    <div className="ml-3">
                                                        <p className="text-sm font-medium text-yellow-600">Local Low Stock</p>
                                                        <p className="text-2xl font-bold text-yellow-900">
                                                            {items.filter(item => item.stock_qty > 0 && item.stock_qty <= 5).length}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="bg-red-50 p-4 rounded-lg">
                                                <div className="flex items-center">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14H5.236a2 2 0 01-1.789-2.894l3.5-7A2 2 0 018.736 4h4.018a2 2 0 01.485.06l3.76.94m-7 10v5a2 2 0 002 2h.096c.5 0 .905-.405.905-.904 0-.715.211-1.413.608-2.008L17.294 15M10 14l4-2-4-2v4z" />
                                                    </svg>
                                                    <div className="ml-3">
                                                        <p className="text-sm font-medium text-red-600">Local Out of Stock</p>
                                            ...
                                            <div className="bg-purple-50 p-4 rounded-lg">
                                                <div className="flex items-center">
                                                    ...
                                                    <div className="ml-3">
                                                        <p className="text-sm font-medium text-purple-600">API Available</p>
                                                        <p className="text-2xl font-bold text-purple-900">
                                                            {isLoadingStocks ? '...' : stockData?.data ? 
                                                                stockData.data.filter(stock => !stock.error && stock.stock_qty > 0).length : '0'
                                                            }
                                                        </p>
                                                    </div>
                                                </div>
                                    ...
                                    {isLoadingItems ? (
                                        ...
                                        <div className="bg-gray-50 p-8 rounded-lg text-center">
                                            ...
                                            <button
                                                onClick={() => setIsModalOpen(true)}
                                                className="bg-primary hover:bg-primary-700 text-white py-2 px-4 rounded-lg shadow-md hover:shadow-lg transition-all">
                                                Add Your First Item
                                            </button>
                                        </div>
                                    ) : (
                                        ...
                                        <div className="mb-2">
                                            ...
                                            {/* API Stock (from stock API) */}
                                            {(() => {
                                                const stockInfo = stockData?.data?.find(stock => stock.item_detail_ID === item.item_id)
                                                return (
                                                            ...
                                                            {isLoadingStocks ? (
                                                                ...
                                                                <>
                                                                    <span className={`ml-1 text-sm font-medium ${
                                                                        stockInfo.error ? 'text-red-600' :
                                                                        stockInfo.stock_qty > 10 ? 'text-green-600' : 
                                                                        stockInfo.stock_qty > 0 ? 'text-yellow-600' : 
                                                                        'text-red-600'
                                                                    }`}>
                                                                        {stockInfo.error ? 'N/A' : (stockInfo.stock_qty !== null ? stockInfo.stock_qty : 'N/A')}
                                                                    </span>
                                                                    {stockInfo.error && (
                                                                        <span className="ml-1 text-xs text-red-500" title={stockInfo.error}>⚠️</span>
                                                                    )}
                                                                </>
                                                            ) : (
                                                                <span className="ml-1 text-sm text-gray-500">Not fetched</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                )
                        ...
                        <form onSubmit={handleSubmit} className="p-6">
                            ...
                            <div className="mt-8 flex justify-end gap-3">
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 bg-primary rounded-md hover:bg-primary-700 disabled:opacity-70 flex items-center"
                                >
                                    {isSubmitting && (
