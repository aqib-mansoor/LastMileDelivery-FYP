...
function RiderAssignedOrders() {
    ...
    const handleViewOnMap = (order) => {
        ...
        // Clear flyToPosition after animation completes to prevent interference
        setTimeout(() => {
    ...
    const renderOrderCard = (order) => {
        ...
        return (
            ...
            <div className="p-4 bg-gray-50">
                <div className="flex justify-between items-center">
                    <div>
                        <p className="text-gray-600 text-sm">Subtotal: PKR {order.total_amount}</p>
                        <p className="text-gray-600 text-sm">Delivery: PKR {config.deliveryFee}</p>
                        <p className="text-lg font-semibold">Total: PKR {(parseFloat(order.total_amount) + config.deliveryFee).toFixed(2)}</p>
                    </div>
        ...
        <div className="relative h-96 w-full rounded-lg overflow-hidden border border-gray-300">
                                    ...
                                    <div className="p-2 max-w-[200px]">
                                        ...
                                        <div className="flex justify-between items-center mt-1">
                                            <div className="text-xs">
                                                <p className="font-semibold">PKR {(parseFloat(order.total_amount) + config.deliveryFee).toFixed(2)}</p>
                                                <p className="text-gray-500">+PKR {config.deliveryFee} delivery</p>
                                            </div>
                                            <button
            ...
            {selectedOrder ? (
                <div className="absolute top-4 left-4 bg-white p-3 rounded-lg shadow-md z-[500] max-w-xs">
                    ...
                    <p className="text-sm font-semibold text-primary">PKR {(parseFloat(selectedOrder.total_amount) + config.deliveryFee).toFixed(2)}</p>
                </div>
                ...
                <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-white py-2 px-4 rounded-full shadow-md">
                    <p className="font-bold text-gray-800">
                        {getFilteredOrders().length > 1 
                            ...
                            : `PKR ${(parseFloat(getFilteredOrders()[0].total_amount) + config.deliveryFee).toFixed(2)}`
                        }
                    </p>
                </div>
            )}
            ...
            <div className="absolute top-1/4 left-1/2 transform -translate-x-1/2 z-[1000]">
                <div className="bg-white rounded-lg shadow-xl w-64">
                    ...
                    <div className="px-3 pb-3">
                        ...
                        <div className="flex justify-between items-center">
                            <p className="font-medium">PKR {(parseFloat(selectedOrder.total_amount) + config.deliveryFee).toFixed(2)}</p>
                            <button
                                className="px-3 py-1 bg-red-500 text-white text-xs rounded hover:bg-red-600"
                                                            onClick={() => {
                                setSelectedOrder(null);
