...
function RiderLayout() {
  ...
  const delieryBoy = localStorage.getItem('deliveryboy') && JSON.parse(localStorage.getItem('deliveryboy')) || null
  const [status, setStatus] = useState(delieryBoy?.status || "Available")
  ...
  const handleStatusChange = async (id) => {
    const response = await fetch(`${config.baseUrl}/deliveryboys/status/${id}`, {
      method: 'PUT',
    })
    ...
    if(response.ok){
      ...
      console.log(data)
      const newStatus = data.delivery_boy?.new_status;
      setStatus(newStatus)

      const previousData = JSON.parse(localStorage.getItem('deliveryboy'))
      const updatedData = {
        ...previousData,
        status: newStatus
      }
      localStorage.setItem('deliveryboy', JSON.stringify(updatedData))
    }
  }
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="hidden md:flex md:flex-shrink-0 md:sticky md:top-0 md:h-screen">
        <div className="flex flex-col w-64 bg-primary-800 text-primary h-screen">
          <div className="flex-shrink-0 flex items-center justify-center h-16 px-4 bg-primary-900">
          ...
          <div className="flex flex-col flex-grow px-4 py-4 min-h-0">
            ...
            {
              <div className="flex-shrink-0 pt-4 mt-6 border-t border-primary-700 flex items-center justify-between">
                <div clas>
                  Status:
                </div>
                <button
                onClick={() => handleStatusChange(delieryBoy.delivery_boy_id)}
                className={
                  status == "Available" ? "bg-green-500 text-white px-4 py-2 rounded-md" : "bg-red-500 text-white px-4 py-2 rounded-md"
                }>
                  {
        ...
        <div className="md:hidden bg-primary-800 text-primary">
          ...
          {isMenuOpen && (
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              <button
                onClick={handleLogout}
                className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-primary hover:bg-primary-700"
              >
                Logout
              </button>
            </div>
