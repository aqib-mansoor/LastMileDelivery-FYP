...
function VendorApproval() {
    ...
    const handleApprove = async (vendor) => {
        ...
        try {
            const response = await fetch(`${config.baseUrl}/admin/vendors/${vendor.id}/approve`, {
                method: 'PUT',
                headers: {
        ...
        // Update local state
            const updatedVendors = vendors.map(v =>
                v.id === vendor.id ? { ...v, approval_status: 'approved' } : v
            )
    ...
    const handleReject = async () => {
        ...
        try {
            const response = await fetch(`${config.baseUrl}/admin/vendors/${selectedVendor.id}/reject`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            // Update local state
            const updatedVendors = vendors.map(v =>
                v.id === selectedVendor.id ? { ...v, approval_status: 'rejected', rejectReason } : v
            )
            setVendors(updatedVendors)
