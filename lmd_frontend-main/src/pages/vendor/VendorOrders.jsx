                            ...
                            <div key={shopId} className="bg-white rounded-lg shadow-md overflow-hidden">
                                ...
                                >
                                    <div className="flex items-center">
                                        <h2 className="text-lg font-semibold">{shops[shopId]?.name || `Shop #${shopId}`}</h2>
                                        <span className="ml-2 text-sm text-gray-500">({getOrdersForShop(shopId).length} orders)</span>
                                    </div>
                                    <svg
                                        className={`w-5 h-5 transform transition-transform ${expandedShops[shopId] ? 'rotate-180' : ''}`}
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                    >
                                                            ...
                                                            {order.suborders
                                                                ...
                                                                .map(suborder => (
                                                                    <span 
                                                                        ...
                                                                        className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(suborder.status)}`}
                                                                    >
                                                                        {suborder.status}
                                                                    </span>
                                                                ))}
