...
function OrderTracking() {
    ...
    useEffect(() => {
        ...
        const fetchTracking = async () => {
            try {
                setTrackingError('')
                const res = await fetch(`${config.baseUrl}/customer/live-tracking/${suborderId}`, {
                ...
                setTrackingLocation([parseFloat(data.data.latitude), parseFloat(data.data.longitude)])
            ...
            <div className="max-w-7xl mx-auto px-4 py-6">
                ...
                <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                    ...
                    <div className="relative">
                        {trackingLocation ? (
                            <MapContainer 
                                center={mapInitialized ? undefined : trackingLocation} 
                                zoom={15} 
                            ...
                            >
                                ...
                                {/* Rider Marker */}
                                <Marker position={trackingLocation} icon={riderIcon}>
                                    <Popup>
                                        <div className="text-sm">
                                            <p className="font-semibold text-blue-600">Rider's Location</p>
                                            <p className="text-gray-600">
                                                Last updated: {lastUpdated ? lastUpdated.toLocaleTimeString() : 'Just now'}
                                            </p>
                                            <p className="text-xs text-gray-500 mt-1">
                                                Lat: {trackingLocation[0].toFixed(6)}<br/>
                                                Lng: {trackingLocation[1].toFixed(6)}
                                            </p>
                                        </div>
                                    </Popup>
                                </Marker>
                                ...
                                {trackingDeliveryLocation && (
                                    <>
                                        ...
                                        {/* Path between rider and delivery location */}
                                        <Polyline 
                                            positions={[trackingLocation, trackingDeliveryLocation]}
                                            color="#3388ff"
                                            weight={3}
                                            opacity={0.7}
                                            dashArray="5, 10"
